	...
	private void initialize(ArrayList<EditorOfJournal> eojs, JFrame editorWindow) {
		...
		btnUpdate.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent arg0) {
				...
				/*
				 * for (EditorOfJournal e: j.getBoardOfEditors()) { if
				 * ((e.getEditor().getFullName()).equals(selectedJournal)) { e.setChiefEditor();
				 * Database.removeChiefEditor(editor.getEditorId()); process = true; } } if
				 * (process) { JOptionPane.showMessageDialog(null, "Transfer Successful",
				 * "Transfer", 1); } else { JOptionPane.showMessageDialog(null,
				 * "Transfer Unsuccessful", "Transfer", 1); }
				 * frmRetireFromWhichJournal.dispose(); new LoginScreen();
				 */
			}
		});
