...
public class AddJournal {
	...
	private void initialize(Academic[] roles) {
		...
		btnAddJournal.addMouseListener(new MouseAdapter() {
			...
			public void mousePressed(MouseEvent arg0) {
				...
				if (journalName.length() == 0 && JournalISSN.length() == 0) {
					JOptionPane.showMessageDialog(panel, "Please enter a journal name and a journal ISSN", "Error",
							JOptionPane.WARNING_MESSAGE);
					validCredentials = false;
				} else if (journalName.length() == 0) {
					JOptionPane.showMessageDialog(panel, "Please enter a journal name", "Error",
							JOptionPane.WARNING_MESSAGE);
					validCredentials = false;
					...
					JOptionPane.showMessageDialog(panel, "Please enter an ISSN of the correct length", "Error",
							JOptionPane.WARNING_MESSAGE);
					validCredentials = false;
				} else {
					try {
						issn = Integer.parseInt(JournalISSN);
						if (Database.validateJournalISSN(issn) && validCredentials) {
							validCredentials = false;	
							JOptionPane.showMessageDialog(panel,
				...
				if (Database.validateJournalTitle(journalName) && validCredentials) {
		...
		groupLayout.setVerticalGroup(groupLayout.createParallelGroup(Alignment.LEADING)
				.addGroup(groupLayout.createSequentialGroup().addContainerGap().addComponent(lblAddANew).addGap(20)
						...
						.addGap(10).addComponent(btnAddJournal).addContainerGap(348, Short.MAX_VALUE)));
